...
int draw_char(monitor_t *mon, font_t *cur_font, int x, int align, uint32_t ch) {
	int ch_width;
...
bool font_has_glyph(font_t *font, const uint32_t c) {
	if (font->xft_ft) {
...
font_t *select_drawable_font(const uint32_t c) {
...
void font_load(const char *pattern) {
	...
	if (!xcb_request_check(c, cookie)) {
		...
		ret->char_max =
			font_info->max_byte1 << 8 | font_info->max_char_or_byte2;
		ret->char_min =
			font_info->min_byte1 << 8 | font_info->min_char_or_byte2;
...
void monitor_create_chain(xcb_rectangle_t *rects, const int num) {
	...
	for (i = 0; i < num; i++) {
		...
		if (rects[i].width > left) {
			monitor_t *mon =
				monitor_new(rects[i].x + left, rects[i].y,
							min(width, rects[i].width - left), rects[i].height);
