...
void fill_rect(xcb_drawable_t d, xcb_gcontext_t _gc, int x, int y, int width,
			   int height) {
	xcb_poly_fill_rectangle(c, d, _gc, 1,
							(const xcb_rectangle_t[]){{x, y, width, height}});
}

// Apparently xcb cannot seem to compose the right request for this call, hence
// we have to do it by ourselves. The funcion is taken from 'wmdia'
// (http://wmdia.sourceforge.net/)
...
int draw_char(monitor_t *mon, font_t *cur_font, int x, int align, uint16_t ch) {
	...
	xcb_poly_text_16_simple(c, mon->pixmap, gc[GC_DRAW], x,
							bh / 2 + cur_font->height / 2 - cur_font->descent,
							1, &ch);
...
rgba_t parse_color(const char *str, char **end, const rgba_t def) {
	...
	if (tmp.a) {
		// The components are clamped automagically as the rgba_t is made of
		// uint8_t
		return (rgba_t){
			.r = (tmp.r * tmp.a) / 255,
			.g = (tmp.g * tmp.a) / 255,
...
bool area_add(char *str, const char *optend, char **end, monitor_t *mon,
	...
	if (*str != ':') {
		*end = str;
		...
		for (i = area_stack.index - 1; i >= 0 && !area_stack.ptr[i].complete;
			 i--)
			;
		a = &area_stack.ptr[i];
	...
	if (area_stack.index + 1 > area_stack.alloc) {
		area_stack.ptr =
			xreallocarray(area_stack.ptr, area_stack.index + 1, sizeof(area_t));
		area_stack.alloc += 1;
	}
...
int pos_to_absolute(monitor_t *mon, int pos, int align) {
	switch (align) {
	case ALIGN_L:
		return pos;
	case ALIGN_R:
...
void font_load(const char *pattern) {
	...
	size_t lut_size =
		sizeof(xcb_charinfo_t) * xcb_query_font_char_infos_length(font_info);
	if (lut_size) {
	...
	if (!font_list) {
		fprintf(stderr, "Failed to allocate %d font descriptors",
				font_count + 1);
		exit(EXIT_FAILURE);
	}
...
monitor_t *monitor_new(int x, int y, int width, int height, char *name) {
	monitor_t *ret;
...
int mon_sort_cb(const void *p1, const void *p2) {
...
void monitor_create_chain(monitor_t *mons, const int num) {
	int i;
	int width = 0, height = 0;
	int left = bx;

	// Sort before use, but only if specific outputs were not specified on
	// command line
	if (!num_outputs)
	...
	for (i = 0; i < num; i++) {
		...
		if (mons[i].width > left) {
			monitor_t *mon = monitor_new(
				mons[i].x + left, mons[i].y, min(width, mons[i].width - left),
				mons[i].height, mons[i].name ? xstrdup(mons[i].name) : NULL);
...
void get_xinerama_monitors(void) {
	...
	if (num_outputs) {
		fprintf(stderr,
				"Using output names with Xinerama is not yet supported\n");
		return;
	}
...
void parse_output_string(char *str) {
	if (!str || !*str)
		return;
...
void init(char *wm_name) {
	...
	for (monitor_t *mon = monhead; mon; mon = mon->next) {
		...
		// Make sure that the window really gets in the place it's supposed to
		// be Some WM such as Openbox need this
		xcb_configure_window(
			c, mon->window,
			XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y |
				XCB_CONFIG_WINDOW_STACK_MODE,
			(const uint32_t[]){mon->x, mon->y, XCB_STACK_MODE_BELOW});
...
int main(int argc, char **argv) {
	...
	while ((ch = getopt(argc, argv, "hg:o:bdf:a:pu:B:F:U:n:")) != -1) {
		...
		case 'h':
			...
			printf("usage: %s [-h | -g | -o | -b | -d | -f | -p | -n | -u | -B "
				   "| -F]\n"
				   "\t-h Show this help\n"
				   ...
				   "\t-d Force docking (use this if your WM isn't EWMH "
				   "compliant)\n"
				   "\t-f Set the font name to use\n"
				   "\t-p Don't close after the data ends\n"
				   "\t-n Set the WM_NAME atom to the specified value for this "
				   "bar\n"
				...
				while ((ev = xcb_poll_for_event(c))) {
					...
					case XCB_BUTTON_PRESS:
						press_ev = (xcb_button_press_event_t *)ev;
						{
							...
							if (area) {
								(void)write(STDOUT_FILENO, area->cmd,
											strlen(area->cmd));
